...
draw_title() {
  local key="$1"
  echo -e "${CYAN}${LANG_LINE_BOX}"
  echo -e "${CYAN}$(eval "echo \$LANG_TITLE_${key}")"
  echo -e "${CYAN}${LANG_LINE_BOX}${NC}"
}

separator() {
  echo -e "${CYAN}${LANG_SEPARATOR}${NC}"
}
...
main_menu() {
  ...
  echo
  echo "${LANG_LINE_BOX}"
  echo -e "${LANG_MENU_HEADER:-XRDP MENU}"
  echo "${LANG_LINE_BOX}"
...
menu_extension() {
  # VSCode extension wizard CLI, tanpa file eksternal
  clear
  draw_title MENU_EXTENSION
  ...
  while IFS= read -r line; do
    ...
    elif [[ "$line" =~ ^([a-zA-Z0-9.-]+)[[:space:]]+\(publisher:[[:space:]]*([^)]+)\)$ ]]; then
